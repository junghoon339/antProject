...
public class UserAuthenticationProvider implements AuthenticationProvider {
	...
	public Authentication authenticate(Authentication auth) throws AuthenticationException {
		...
		if (userDTO == null) { // ID가 없는 경우
			throw new UsernameNotFoundException("아이디 또는 비밀번호를 다시 확인하세요. <br>등록되지 않은 아이디이거나, 아이디 또는 비밀번호를 잘못 입력하셨습니다.");
		}
		...
		if (!passwordEncoder.matches(password, userDTO.getUserPassword())) {
			throw new BadCredentialsException("아이디 또는 비밀번호를 다시 확인하세요. <br>등록되지 않은 아이디이거나, 아이디 또는 비밀번호를 잘못 입력하셨습니다.");
		}
